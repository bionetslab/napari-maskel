...
Inside the widget, tune extraction settings and use **Save Config** to export a reusable JSON preset — the same preset the [maskel CLI](https://github.com/bionetslab/maskel) consumes for batch processing (`maskel run --config`). The widget scrolls, so it's never clipped by the dock panel's height; the less-frequently-used **Cleanup** and **Advanced Features** groups are collapsed by default (click their headers to expand).
