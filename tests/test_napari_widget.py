...
def test_fractal_anisotropic_warning_shown_for_anisotropic_spacing(widget):
    _select_image(widget, _mock_layer((10, 10)))
    widget.include_fractal_widget.value = True
    widget.spacing_widget.value = "1.0,0.5"
...
class TestOnAnalyze:
    """`_on_analyze` runs the real maskel pipeline (not mocked) on a small
    real mask - the widget's own docstring already noted this method had
    no coverage; these run the actual analysis end to end rather than
    stubbing it out, the same way tests/test_napari_layers.py already does
    for the layer-extraction half of the same workflow."""

    def test_no_image_selected_shows_info_and_adds_no_layers(self, widget, monkeypatch):
        show_info = MagicMock()
        monkeypatch.setattr(_napari, "show_info", show_info)

        widget._on_analyze()

        show_info.assert_called_once()
        assert "select a label layer" in show_info.call_args.args[0].lower()
        widget.viewer.add_layer.assert_not_called()

    def test_happy_path_reports_pixel_counts_and_adds_layers(self, widget, monkeypatch):
        show_info = MagicMock()
        monkeypatch.setattr(_napari, "show_info", show_info)
        _select_image(widget, _real_image_layer(_cross_mask()))

        widget._on_analyze()

        messages = [c.args[0] for c in show_info.call_args_list]
        assert any("skeleton pixels" in m for m in messages)
        assert widget.viewer.add_layer.call_count >= 1

    def test_show_preprocessed_adds_an_extra_layer(self, widget, monkeypatch):
        monkeypatch.setattr(_napari, "show_info", MagicMock())
        _select_image(widget, _real_image_layer(_cross_mask(), name="obj"))
        widget.fill_holes_widget.value = True
        widget.show_preprocessed_widget.value = True
        ...
        assert "obj_preprocessed" in added_names

    def test_one_bad_layer_does_not_abort_the_others(self, widget, monkeypatch):
        show_info = MagicMock()
        monkeypatch.setattr(_napari, "show_info", show_info)
        _select_image(widget, _real_image_layer(_cross_mask(), name="obj"))
        # An unrecognized layer_type makes Layer.create raise for just this
        # one entry - a real failure mode (a future maskel/napari version
        # skew producing a layer_type this napari doesn't know), without
        # needing to monkeypatch napari's own Layer.create descriptor.
        monkeypatch.setattr(
            _napari,
        ...
        messages = [c.args[0] for c in show_info.call_args_list]
        assert any("Failed to add layer obj_bad" in m for m in messages)
        assert widget.viewer.add_layer.call_count == 1

    def test_output_dir_set_writes_real_files_to_disk(
        ...
        assert (tmp_path / "obj" / "obj_skeleton.npy").exists()
        assert (tmp_path / "obj" / "obj_summary.csv").exists()

    def test_pipeline_error_is_reported_as_analysis_failed(self, widget, monkeypatch):
        show_error = MagicMock()
        monkeypatch.setattr(_napari, "show_error", show_error)
        _select_image(widget, _real_image_layer(_cross_mask()))
        monkeypatch.setattr(
            _napari,
            "analyze_segmentation_mask",
            MagicMock(side_effect=ValueError("bad mask")),
        )

        widget._on_analyze()

        show_error.assert_called_once()
        assert show_error.call_args.args[0].startswith("Analysis failed:")

    def test_save_failure_after_successful_analysis_is_reported_distinctly(
        self, widget, monkeypatch, tmp_path
    ):
        show_error = MagicMock()
        monkeypatch.setattr(_napari, "show_error", show_error)
        ...
        # analysis itself succeeded - layers were already added - despite
        # the save failing, so the message says so instead of claiming the
        # analysis failed.
        assert widget.viewer.add_layer.call_count >= 1
        show_error.assert_called_once()
        assert show_error.call_args.args[0].startswith("Analysis succeeded")


# -- config load / save / output-dir dialogs ----------------------------------


class TestLoadConfigDialog:
    def test_cancel_is_a_noop(self, widget, monkeypatch):
        monkeypatch.setattr(QFileDialog, "getOpenFileName", lambda *a, **k: ("", ""))
        show_info, show_error = MagicMock(), MagicMock()
        monkeypatch.setattr(_napari, "show_info", show_info)
        monkeypatch.setattr(_napari, "show_error", show_error)
    ...
    def test_success_updates_widget_state(self, widget, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            QFileDialog, "getOpenFileName", lambda *a, **k: (str(config_path), "")
        )
        show_info = MagicMock()
        monkeypatch.setattr(_napari, "show_info", show_info)
        ...
        show_info.assert_called_once_with("Configuration loaded")

    def test_malformed_recipe_shows_error_instead_of_raising(
        self, widget, monkeypatch, tmp_path
    ):
        # Valid JSON, wrong structure ("extraction" must be an object) -
        # PipelineConfig.from_dict raises TypeError for this, which must be
        # caught the same way as a ValueError/OSError, not propagate.
        config_path = tmp_path / "bad.json"
        config_path.write_text(
            json.dumps({"schema_version": 6, "extraction": "oops", "output": {}})
        )
        monkeypatch.setattr(
            QFileDialog, "getOpenFileName", lambda *a, **k: (str(config_path), "")
        )
        ...
        show_error.assert_called_once()
        assert "Failed to load config" in show_error.call_args.args[0]


class TestSaveConfigDialog:
    def test_cancel_is_a_noop(self, widget, monkeypatch, tmp_path):
    ...
    def test_success_writes_a_real_file(self, widget, monkeypatch, tmp_path):
        ...
        show_info.assert_called_once()
        assert str(out_path) in show_info.call_args.args[0]

    def test_write_failure_shows_error(self, widget, monkeypatch, tmp_path):
        out_path = tmp_path / "saved.json"
        monkeypatch.setattr(
            QFileDialog, "getSaveFileName", lambda *a, **k: (str(out_path), "")
        )
        monkeypatch.setattr(
            _napari, "save_pipeline_config", MagicMock(side_effect=OSError("nope"))
        )
        show_error = MagicMock()
        monkeypatch.setattr(_napari, "show_error", show_error)
        ...
        show_error.assert_called_once()
        assert "Failed to save config" in show_error.call_args.args[0]


class TestSelectOutputDirDialog:
    ...
    def test_success_sets_path_and_updates_button(self, widget, monkeypatch, tmp_path):
        monkeypatch.setattr(
...
def _branches_shapes_layer(name: str, properties: dict, n_shapes: int) -> Shapes:
    data = [np.array([[i, 0], [i, 1]]) for i in range(n_shapes)]
    return Shapes(data=data, shape_type="line", properties=properties, name=name)


class TestRecolorBranchLayers:
    def test_non_shapes_layers_are_skipped(self, widget):
        labels_layer = MagicMock()
        labels_layer.name = "obj_branches"
        widget.viewer.layers = [labels_layer]

        widget._recolor_branch_layers()  # must not raise

    def test_shapes_layers_with_wrong_name_suffix_are_untouched(self, widget):
        layer = _branches_shapes_layer("obj_nodes", {"tortuosity": [1.0, 2.0]}, 2)
        before = layer.edge_color.copy()
        widget.viewer.layers = [layer]
        widget.branch_color_widget.value = "tortuosity"

        widget._recolor_branch_layers()

        assert np.array_equal(layer.edge_color, before)

    def test_recolors_by_numeric_property_with_real_spread(self, widget):
        layer = _branches_shapes_layer(
            "obj_branches", {"tortuosity": [1.0, 2.0, 3.0]}, 3
        )
        widget.viewer.layers = [layer]
        widget.branch_color_widget.value = "tortuosity"
        ...
        assert layer.edge_contrast_limits == (1.0, 3.0)

    def test_falls_back_to_flat_teal_when_property_has_no_spread(self, widget):
        layer = _branches_shapes_layer(
            "obj_branches", {"tortuosity": [1.0, 1.0, 1.0]}, 3
        )
        widget.viewer.layers = [layer]
        widget.branch_color_widget.value = "tortuosity"
    ...
    def test_falls_back_to_flat_teal_when_property_is_absent(self, widget):
        layer = _branches_shapes_layer("obj_branches", {"other": [1.0, 2.0]}, 2)
        widget.viewer.layers = [layer]
        widget.branch_color_widget.value = "tortuosity"
